...
$.TextboxList = function(element, _options){
	...
	var create = function(klass, value, opt){
		...
		if (opt){		
			...
			opt.style='margin:0;border:0;font-size:12px';		
		}
...
$.TextboxListBit = function(type, value, textboxlist, _options){
	...
	var init = function(){
		...
		if (type == 'box'){
			...
			element = $('<input type="text" class="'+ typeprefix +'-input" autocomplete="off" lang="'+options.lang+'" style="'+options.style+'"/>').val(self.value ? self.value[1] : '').appendTo(bit);
