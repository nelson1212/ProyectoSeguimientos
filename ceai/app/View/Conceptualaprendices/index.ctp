<div class="conceptualaprendices index">
	<h2><?php echo __('Conceptualaprendices');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('aprendice_id');?></th>
			<th><?php echo $this->Paginator->sort('acta_id');?></th>
			<th><?php echo $this->Paginator->sort('concepto');?></th>
			<th><?php echo $this->Paginator->sort('created');?></th>
			<th><?php echo $this->Paginator->sort('updated');?></th>
			<th class="actions"><?php echo __('Actions');?></th>
	</tr>
	<?php
	foreach ($conceptualaprendices as $conceptualaprendice): ?>
	<tr>
		<td><?php echo h($conceptualaprendice['Conceptualaprendice']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($conceptualaprendice['Aprendice']['id'], array('controller' => 'aprendices', 'action' => 'view', $conceptualaprendice['Aprendice']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($conceptualaprendice['Acta']['id'], array('controller' => 'actas', 'action' => 'view', $conceptualaprendice['Acta']['id'])); ?>
		</td>
		<td><?php echo h($conceptualaprendice['Conceptualaprendice']['concepto']); ?>&nbsp;</td>
		<td><?php echo h($conceptualaprendice['Conceptualaprendice']['created']); ?>&nbsp;</td>
		<td><?php echo h($conceptualaprendice['Conceptualaprendice']['updated']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $conceptualaprendice['Conceptualaprendice']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $conceptualaprendice['Conceptualaprendice']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $conceptualaprendice['Conceptualaprendice']['id']), null, __('Are you sure you want to delete # %s?', $conceptualaprendice['Conceptualaprendice']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Conceptualaprendice'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Aprendices'), array('controller' => 'aprendices', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Aprendice'), array('controller' => 'aprendices', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Actas'), array('controller' => 'actas', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Acta'), array('controller' => 'actas', 'action' => 'add')); ?> </li>
	</ul>
</div>
